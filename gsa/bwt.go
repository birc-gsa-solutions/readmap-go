...
// BuildFMIndexApproxTables builds the preprocessing tables for
// approximative FM-index searching.
func BuildFMIndexApproxTables(x string) *FMIndexTables {
	...
	revsa, _ := SaisWithAlphabet(revx, alpha)
	revb, _ := alpha.MapToBytesWithSentinel(revx)
	rotab := NewOTab(Bwt(revb, revsa), alpha.Size())
...
func fmApproxReport(left, right int, ops *EditOps, sa *[]int32, cb func(i int32, cigar string)) {
...
func FMIndexApproxFromTables(tbls *FMIndexTables) func(p string, edits int, cb func(i int32, cigar string)) {
	return func(p string, edits int, cb func(i int32, cigar string)) {
...
func FMIndexApproxPreprocess(x string) func(p string, edits int, cb func(i int32, cigar string)) {
